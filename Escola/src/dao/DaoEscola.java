...
public class DaoEscola {
    ...
    public static boolean alterar(Escola objeto) {
        String sql = "UPDATE escola SET nome = ?, endereco = ?, sigla = ?, numeroalunos = ?, area = ? WHERE codigo=?";
        try {
            PreparedStatement ps = conexao.Conexao.getConexao().prepareStatement(sql);
    ...
    public static List<Escola> consultar() {
        ...
        String sql = "SELECT codigo, nome, endereco, sigla, numeroalunos, area FROM escola";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                objeto.setNumeroalunos(rs.getInt("numeroalunos"));
